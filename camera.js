...
const speak = (msg) => speechSynthesis.speak(new SpeechSynthesisUtterance(msg));

// show the initial countdown
const interval = setInterval(() => {
  if (msgIndex > readyMessages.length - 1) {
    clearInterval(interval);
    textOutput.innerText = numSquats;
    textOutput.style.display = 'inline-block';
    textMsg.style.display = 'none';
    return;
  }
  textOutput.style.display = 'none';
  textMsg.innerText = readyMessages[msgIndex++];
}, 1000);

// TODO: 
// - track hips to increase accuracy
// - gamification:
//   - yell slurs when failing daily goals
//   - auto post milestone achievements to facebook

// count squats
const countSquats = keypoints => {
  // get the y positions of body parts
  const leftEyeYPos = keypoints[1].position.y;
  const rightEyeYPos = keypoints[2].position.y;

  const leftShoulderYPos = keypoints[5].position.y;
  const rightShoulderYPos = keypoints[6].position.y;
  
  if (squatStep === 0 && leftEyeYPos < squatStartHeight && rightShoulderYPos < squatStartHeight) {
    squatStep = 1;
    // console.log(squatStep);
  } else if (squatStep === 1 && leftEyeYPos > squatEndHeight && rightShoulderYPos > squatEndHeight) {
    squatStep = 2;
    // console.log(squatStep);
  } else if (squatStep === 2 && leftEyeYPos < squatEndHeight && rightShoulderYPos < squatEndHeight) {
    squatStep = 0;
    numSquats++;
    localStorage.setItem('numSquats', numSquats);
    textOutput.innerHTML = numSquats;
    speak(numSquats);
    // console.log(squatStep);
  } else {
  ...
  async function poseDetectionFrame() {
    ...
    poses.forEach(({ score, keypoints }) => {
      if (score >= minPoseConfidence) {
        // hook into demo app and start counting
